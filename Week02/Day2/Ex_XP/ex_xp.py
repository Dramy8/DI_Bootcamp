...
favorite_book(title = "Harry Potter and the deathly hollows")
# %%
#EX3
...
describe_city(city="Lyon", country= "France")
...
def randnum(my_num):
    ...
    if my_num == the_num:
        ...
        print(f"Fail! Your number: {my_num}, random number: {the_num}")
...
# Modify the make_shirt() function so that size has a default value of “large” 
# and text has a default value of “I love Python”.
...
make_shirt(size="small",text="Central Perk")
...
# Create a function called get_random_temp() that returns
#  a random integer between -10 and 40 degrees Celsius.
...
import random
def get_random_temp():
    month = int(input("Enter a month in numerical value (1-12): "))
    if month == 12 or 1<=month<=2:
        rand_nb = round(random.uniform(-10,14),2)
        return rand_nb
    elif 3<=month<=5:
        rand_nb = round(random.uniform(15,23),2)
        return rand_nb
    elif 6<=month<=8:
        rand_nb = round(random.uniform(20,32),2)
        return rand_nb
    elif 9<=month<=11:
        rand_nb = round(random.uniform(11,18),2)
        return rand_nb
    else:
