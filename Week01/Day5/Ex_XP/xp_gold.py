#%%
#EX1
# Instructions
# Create a variable called birthdays. Its value should be a dictionary.
# Initialize this variable with birthdays of 5 people of your choice. 
# For each entry in the dictionary, the key should be the person’s name,
# and the value should be their birthday. Tip : Use the format “YYYY/MM/DD”.
# Print a welcome message for the user. 
# Then tell them: “You can look up the birthdays of the people in the list!”“
# Ask the user to give you a person’s name and store the answer in a variable.
# Get the birthday of the name provided by the user.
# Print out the birthday with a nicely-formatted message.

birthdays = {
    'dad': '1959/10/28',
    'mom': '1964/03/22',
    'dodie': '1987/02/17',
    'nanou': '1991/09/28',
    'yayoun': '1995/07/04'
}

print("Welcome to the family birth dates menu. You can look up the birthdays of the people in the list!")
name = input("Enter one of the person's name: ")

if name in birthdays:
    print(f"{name}'s birthday is {birthdays[name]}")
else:
    print("Sorry, this person is not in the list.")


# %%
#EX2
# Instructions
# Before asking the user to input a person’s name print out all of the names in the dictionary.
# If the person that the user types is not found in the dictionary, 
# print an error message (“Sorry, we don’t have the birthday information for <person’s name>”)

birthdays = {
    'dad': '1959/10/28',
    'mom': '1964/03/22',
    'dodie': '1987/02/17',
    'nanou': '1991/09/28',
    'yayoun': '1995/07/04'
}
print("Welcome to the family birth dates menu. You can look up the birthdays of the people in the list!")
print("Here are the people you can choose from:")
for name in birthdays:
    print(name)
name = input("Enter one of the person's name: ")

if name in birthdays:
    print(f"{name}'s birthday is {birthdays[name]}")
else:
    print(f"Sorry, we don’t have the birthday information for {name}")

# %%
#EX3
# Add this new code: before asking the user to input a person’s name to look up, 
# ask the user to add a new birthday:
# Ask the user for a person’s name – store it in a variable.
# Ask the user for this person’s birthday (in the format “YYYY/MM/DD”) - store it in a variable.
# Now add this new data into your dictionary.
# Make sure that if the user types any name that exists in the dictionary – 
# including the name that he entered himself – the corresponding birthday is found and displayed.

birthdays = {
    'dad': '1959/10/28',
    'mom': '1964/03/22',
    'dodie': '1987/02/17',
    'nanou': '1991/09/28',
    'yayoun': '1995/07/04'
}

new_name = input("Enter a new person's name: ")
new_date = input("Enter the new person birthday in the format YYYY/MM/JJ: ")
birthdays[new_name] = new_date

print("Welcome to the family birth dates menu. You can look up the birthdays of the people in the list!")
print("Here are the people you can choose from:")
for n in birthdays:
    print(n)

name = input("Enter one of the person's name: ")

if name in birthdays:
    print(f"{name}'s birthday is {birthdays[name]}")
else:
    print(f"Sorry, we don’t have the birthday information for {name}")


# %%
#EX4
# Instructions
# items = {
#     "banana": 4,
#     "apple": 2,
#     "orange": 1.5,
#     "pear": 3
# }
# Using the dictionary above, each key-value pair represents an item and its price - 
# print all the items and their prices in a sentence.
# Using the dictionary below, each value are dictionaries containing both the price 
# and the amount of items in stock -
# write some code to calculate how much it would cost to buy everything in stock.
# items = {
#     "banana": {"price": 4 , "stock":10},
#     "apple": {"price": 2, "stock":5},
#     "orange": {"price": 1.5 , "stock":24},
#     "pear": {"price": 3 , "stock":1}
# }

items = {
    "banana": 4,
    "apple": 2,
    "orange": 1.5,
    "pear": 3
}

for key, value in items.items():
    print(f"The price for a {key} is {value}")

items = {
    "banana": {"price": 4 , "stock":10},
    "apple": {"price": 2, "stock":5},
    "orange": {"price": 1.5 , "stock":24},
    "pear": {"price": 3 , "stock":1}
}


#comprehension 

total = sum(data["price"] * data["stock"] for data in items.values())

print(f"It would cost {total} to buy everything in stock")


# %%
